...
- An OpenAI profile MUST NEVER reach `/auth/refresh` (`server.ts:7215`), which would POST a ChatGPT refresh token to the Anthropic token endpoint.
...
- Modify: `src/proxy/server.ts:7215` (`/auth/refresh`), `:7548`, `:7858`
...
- [ ] **Step 5:** Implement. Audit `server.ts:7548` and `:7858` for the same hazard.
...
- [ ] **Step 5:** Add the `@meridian_gpt host meridian-gpt.desktop.ts.nowaker.net` block reverse-proxying to that port, matching the existing `@meridian` / `@meridian_dev` convention.
- [ ] **Step 6:** Verify with `/health` over the tailnet hostname, and confirm the instance did NOT resolve the ambient Anthropic account.
...
4. **Import under an exclusive lease.** Acquire Meridian's writer lease, take a restricted-permission backup of the pool, run `bin/import-codex-pool.ts` atomically. *Verify: six accounts present in the owned store, keyed by `accountUserId`, with the two `accountId`-colliding accounts distinct.*
...
**Irreversible if wrong:** R1, R2, R3, R5 all end in the same place - a ChatGPT account that cannot be recovered by any amount of retrying, only by a human logging in again. Everything else in this plan is a bug; those four are data loss.
