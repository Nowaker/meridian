...
const RENAMED_PROFILES = [
  { id: "employer", aliases: ["work"], type: "oauth-token" as const, oauthToken: "token-work" },
  { id: "personal", type: "oauth-token" as const, oauthToken: "token-personal" },
]
...
function createRenamedTestApp(): TestApp {
  return createProxyServer({ port: 0, host: "127.0.0.1", profiles: RENAMED_PROFILES, defaultProfile: "work" }).app
}
...
describe("profile mutation reconciliation", () => {
  async function withDiskProfiles(
    profiles: Array<{ readonly id: string; readonly type: "oauth-token"; readonly oauthToken: string }>,
    run: (app: TestApp) => Promise<void>,
  ): Promise<void> {
    const configDir = mkdtempSync(join(tmpdir(), "meridian-profile-mutation-"))
    const previousConfigDir = process.env.MERIDIAN_CONFIG_DIR
    process.env.MERIDIAN_CONFIG_DIR = configDir
    writeFileSync(join(configDir, "profiles.json"), `${JSON.stringify(profiles, null, 2)}\n`)
    invalidateDiskProfileCache()
    enableDiskProfileDiscovery()
    try {
      await run(createProxyServer({ port: 0, host: "127.0.0.1" }).app)
    } finally {
      setSetting("routingExcludedProfiles", [])
      setSetting("routingManagedExcludedProfiles", [])
      resetActiveProfile()
      resetDiskProfileDiscovery()
      invalidateDiskProfileCache()
      if (previousConfigDir === undefined) delete process.env.MERIDIAN_CONFIG_DIR
      else process.env.MERIDIAN_CONFIG_DIR = previousConfigDir
      rmSync(configDir, { recursive: true, force: true })
    }
  }

  it("moves active state away from a renamed profile excluded by its former name", async () => {
    await withDiskProfiles([...PROFILES], async (app) => {
      // Given
      setActiveProfile("work")
      setSetting("routingExcludedProfiles", ["work"])

      // When
      const response = await app.fetch(new Request("http://localhost/profiles/rename", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ from: "work", to: "employer" }),
      }))
  ...
  it("skips excluded profiles when replacing a removed active profile", async () => {
    await withDiskProfiles([
      ...PROFILES,
      { id: "backup", type: "oauth-token", oauthToken: "token-backup" },
    ], async (app) => {
      // Given
      setActiveProfile("work")
      setSetting("routingExcludedProfiles", ["personal"])
      ...
      // When
      const response = await app.fetch(new Request("http://localhost/profiles/remove", {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      // Then
      expect(response.status).toBe(200)
      expect(getActiveProfileId()).toBe("backup")
    })
  })
})
...
describe("work routing exclusions", () => {
  ...
  it("does not route work to an excluded followed profile", async () => {
    // Given
    process.env.MERIDIAN_FOLLOW_ACTIVE = "http://127.0.0.1:3456"
  ...
  it("keeps an exclusion attached after the profile is renamed", async () => {
    // Given
    setSetting("routingExcludedProfiles", ["work"])
    const app = createRenamedTestApp()
  ...
  it("rejects an excluded renamed profile requested through its alias", async () => {
    // Given
    setSetting("routingExcludedProfiles", ["employer"])
    const app = createRenamedTestApp()
