...
describe("auth audit: every registered prefix is protected when MERIDIAN_API_KEY is set", () => {
  ...
  //                     less than `/health` beside them, which answers with
  //                     the signed-in email and subscription type to anyone.
  //     why not gated   a 401 is what a load balancer reads as "this backend
  //                     is down". Gating these would make every instance look
  //                     unhealthy the moment MERIDIAN_API_KEY is set, so an
  //                     auth setting would become a total outage of whatever
  //                     sits in front - the failure this pair exists to
  //                     prevent.
  //
