...
describe("proxy async ops", () => {
  ...
  it("serves async health endpoint with correct response schema", async () => {
    ...
    if (body.status === "healthy") {
      ...
      // process.
      const expectedKeys = ["auth", "build", "mode", "plugin", "status", "upstream", "version"]
    ...
    if (body.status === "unhealthy") {
      ...
      expect(Object.keys(body).sort()).toEqual(["auth", "build", "error", "status", "upstream", "version"])
    }

    if (body.status === "degraded") {
      expect(typeof body.error).toBe("string")
      expect(Object.keys(body).sort()).toEqual(["build", "error", "mode", "status", "upstream", "version"])
    }
